...
def MonteCarlo(f, a, b, N, v, p):
    ...
    if p:
        ...
        for i in range(len(pointsX)):
            if (pointsY[i] <= f(pointsX[i])) & (pointsY[i] >= 0) or (
...
def adapQuadRec(func, a0, b0, a, b, tol):
    ...
    if np.abs(Sab - Sac - Scb) < 3 * tol * (b - a) / (b0 - a0):
        ...
        return adapQuadRec(func, a0, b0, a, c, tol) + adapQuadRec(
            func, a0, b0, c, b, tol
        )
...
@click.option(
    ...
    type=str,
    default="AQ",
    help="Method to be used (AQ/trapz/MC)",
)
@click.option(
    "--function", "-f", "func", required=True, type=str, help="Equation to be used"
)
...
@click.option(
    ...
    type=str,
    default="1e-5",
    help="Tolerance to accept answer",
)
@click.option(
    "--verbose",
    "-v",
...
@click.option(
    "--plot", "-p", "plot", count=True, default=False, help="Plots graph of integration"
)
@click.option(
    "--time",
    "-t",
    ...
    count=True,
    default=False,
    help="Prints time used for function to complete, do not use with -p",
)
