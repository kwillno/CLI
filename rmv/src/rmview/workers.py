...
class RFB(RFBClient):
    ...
    def vncConnectionMade(self):
        ...
        self.setEncodings(
            [HEXTILE_ENCODING, CORRE_ENCODING, RRE_ENCODING, RAW_ENCODING]
        )
    ...
    def updateRectangle(self, x, y, width, height, data):
        self.painter.drawImage(
            x, y, QImage(data, width, height, width * BYTES_PER_PIXEL, IMG_FORMAT)
        )
...
class FrameBufferWorker(QRunnable):
    ...
    def stop(self):
        ...
        try:
            ...
            log.warning(
                "Although this is not a big problem, it may consume some resources until you restart the tablet."
            )
            log.warning(
                "You can manually terminate it by running `ssh %s killall rM-vnc-server`.",
                self.ssh.hostname,
            )
            log.error(e)
    ...
    def run(self):
        try:
            _, out, _ = self.ssh.exec_command(
                "/sbin/insmod $HOME/mxc_epdc_fb_damage.ko"
            )
        ...
        while self._stop == False:
            ...
            try:
                self.vncClient = internet.TCPClient(
                    self.ssh.hostname, 5900, RFBFactory(self.signals)
                )
...
class PointerWorker(QRunnable):
    ...
    @pyqtSlot()
    def run(self):
        penkill, penstream, _ = self.ssh.exec_command(
            "cat /dev/input/event0 & { read ; kill %1; }"
        )
