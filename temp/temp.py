...
def plotTemp(hours, humidity, tb, hb):
    ...
    plt.title(
        str(int(selectedTime / (60 ** 2)))
        + " hours before "
        + time.ctime(lastTime)[4:16]
        + time.ctime(lastTime)[19:]
    )
    fig.tight_layout()
    plt.show()
...
@click.option(
    ...
    "plot",
    count=True,
    help="Plot temperature and humidity for selected timeframe",
)
...
@click.option(
    ...
    "humidity",
    count=True,
    help="Show humidity, both in plot and console output",
)
@click.option(
    "--tempbound",
    "-tb",
    "tempbound",
    default=(0.0, 0.0),
    help="Bounds for plotting temperature",
)
@click.option(
    "--humbound",
    "-hb",
    "humbound",
    default=(0.0, 0.0),
    help="Bounds for plotting humidity",
)
